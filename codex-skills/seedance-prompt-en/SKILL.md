...
```json
{
  ...
  "owner_tool": "cu_drag|cu_safari_web|none",
  "refs_visible": false,
...
1. Safari Runway Generate board frontmost; cursor scene matches package.
2. Open one ordered staging folder in Finder (upper-right, not covering Multi-ref/prompt/Generate).
3. `owner_tool=cu_drag`: Computer Use drags **one** file into the empty Multi-ref slot on **Safari**.
4. Verify visible thumbnail count/order in Safari (this is PASS, not the drag log).
...
1. Safari Runway is frontmost Generate board;
...
- Click exact Generate **once** after all checks (Safari Computer Use).
