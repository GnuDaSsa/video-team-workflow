...
One Seedance lane performs two sequential phases in the current conversation. Runtime AGENTS §1.3 owns actual-model attribution and any separately approved optional Astra-author handoff; this skill does not create a permanent author agent or pause for routine spawn approval:
...
2. For prompt design, reference mapping, or CLI handoff, read `seedance-prompting.md`.
   ...
   Read `xazinga-prompting-adapter.md` only when source-scope, reference-driven camera/light, or transition design needs it. Read `hell-grind-production-prompting-adapter.md` only for recurring-character identity, multi-entity blocking, difficult spatial continuity, or a user-declared example precedence. These are optional adapters for the matching risk, not startup reading for an environment-only or otherwise simple shot.
...
- Prefer useful 15-second source yield. A single sustained cinematic shot with edit handles is valid; use a Planner-authored `PLANNED_MULTI_SHOT_SOURCE` of 2–4 scenes only when they are genuinely causally related and do not overload the prompt or reference deck. A short final edit does not by itself shorten the locked provider duration or require multiple generated scenes. Under `GENERAL_REFERENCE_MODE`, reference attachment order is never scene order.
