...
- Capture the browser's real download event and verify its local path. Do not use an ambiguous player/media extractor such as `downloadMedia()` on a feed or viewer with multiple outputs; it can silently retrieve an older card. `Download all` is also not identity-safe for individual completion evidence.
...
- If the exact `All Generations` asset-menu download also reaches `ERR_BLOCKED_BY_CLIENT`, the safe UI ladder is exhausted and the Chrome client unblock below becomes the required human action.
