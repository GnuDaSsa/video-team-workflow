...
Standing defaults: **multi-reference × 15s**, Creative Mode open after identity lock, **Audio ON** with the soundscape directed in the prompt, naturalism-first, medium-aware texture. Shorter duration or dialogue only with an explicit per-shot override. Never switch audio off; music is a normal prompt choice.
...
Default settings line unless overridden: `15s multi-ref; Audio: ON; soundscape directed in the prompt`.
...
7. Keep the card at 15s multi-ref with Audio ON, and state the intended soundscape in the prompt.
...
- default 15s multi-ref recorded, the settings line reads `Audio: ON`, and the prompt names the soundscape;
