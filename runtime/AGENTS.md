...
- **수리 / 보류 / 차단 (2026-07-29)**: 멈추기 전에 한 가지만 묻는다 — **내가 지금 알려진 방법으로 고칠 수 있는가?**
  - **REPAIR**: 내가 만든 입력물의 결함이고 고치는 법이 문서에 있다 → **지금 고치고 진행한다** (시도 2회까지). 프롬프트 텍스트 누락·CJK 유실, 캐릭터시트 미첨부, `PROVIDER_REF` 부재, I2V 소스로 못 쓸 프레임, 덱 중복 — 전부 여기다.
  - **DEFER**: 수리 2회가 실패했다 → 그 패키지만 표시하고 **다음 패키지로 넘어간다**. 세션 끝에 보고.
  - **BLOCKED**: 사람이나 외부 시스템이 있어야 한다(로그인·결제·CAPTCHA·권한·프로바이더 장애) → 1회 기록 + 정확한 조치 안내 + 사용자 보고, 그 항목은 건너뛰고 계속 일한다.
  - **BLOCKED를 폴링하지 않는다** — 폴링은 로그인도 결제도 타이핑도 못 한다. **REPAIR를 기다리지 않는다** — 기다린다고 내 손으로 망친 게 고쳐지지 않는다.
  - 한글 등 비-ASCII 프롬프트는 합성 키입력에서 유실된다. §4.4 Route A(클립보드 붙여넣기 → **보이는 카운터**로 검증, AX tree는 이 에디터에서 오판)로 재삽입하고, 실패 시 Route B(`execCommand insertText`). 2회 실패면 DEFER.
  - 불완전한 걸 알면서 Generate하는 것은 슬롯 낭비다. 그러나 **다른 패키지가 준비돼 있는데 그것만 붙잡고 기다리는 건 더 나쁘다.**

- **계속이 기본이다. 멈추려면 이름을 대야 한다 (2026-07-29)**: 이 파일의 모든 조항에 적용되고, 나중에 추가될 조항과 그 말을 빠뜨린 조항에도 적용된다.
  - **침묵은 계속을 뜻한다.** 어떤 규칙이 무엇을 하지 말라고 하면 그건 *그 행동*에 대한 제한이지 실행을 끝내라는 허가가 아니다. 실패만 적어두고 다음 행동을 안 적은 규칙은 "기록하고 계속하라"는 뜻이다.
  - **턴은 셋 중 하나를 선언해야만 끝난다.** 네 번째는 없고, 그냥 이어가지 않는 방식으로는 끝낼 수 없다.
    - `QUEUE_FULL_WAITING` — 슬롯 만석, 다음 패키지 장전됨. **예약한 다음 확인을 함께 적는다**
    - `SHELF_EXHAUSTED` — 장전할 게 없음. 완료 내역을 적는다
    - `ALL_REMAINING_BLOCKED` — 남은 항목 전부 블로킹. 항목별 사유를 적는다
  - 멈추려는데 셋 중 어디에도 해당하지 않으면 **아직 안 끝난 것**이다. 규칙 하나를 다 읽은 것도, 블록 코드를 만난 것도, 생성 하나를 마친 것도, 지시가 떨어진 것도 종료 사유가 아니다.
  - **왜 이 조항이 필요한가**: 규칙집은 허용보다 금지가 빠르게 쌓인다. 사고마다 "하지 마라"가 하나씩 붙지만 "계속하라"는 아무도 안 붙인다. 그대로 두면 규칙집은 *누락에 의해* 멈추는 쪽으로 흘러간다. 이 조항이 그 표류를 상시 교정한다 — 앞으로 어떤 규칙이 "그 다음"을 안 적어도 여기서 덮인다.
  - **코드가 검사한다**: `validate`가 `cycle_status`로 미완 종료를 잡는다. 레인이 안 돌고 terminal_state 선언도 없는데 선반에 프롬프트가 남아 있으면 `STOPPED_INCOMPLETE` problem이다. `QUEUE_FULL_WAITING`을 선언하고 `next_check_scheduled`가 없어도 마찬가지다.

- **스케줄러는 사용자가 말하지 않아도 건다 (2026-07-29)**: 장전된 상태에서 Generate가 회색이면(=진짜 큐 만석) **다음 확인을 거는 것이 사이클의 필수 단계**다. 재량이 아니다. 큐가 찼는데 확인을 안 걸고 멈추면 그건 일시정지가 아니라 조용한 종료이고, 슬롯이 풀려도 사람이 알아챌 때까지 놀게 된다.
  - 규칙집의 스케줄러 언급은 **금지 7건·허용 3건·의무 0건**이었다. 그 금지들은 *증식*(두 번째 브라우저 루프·사이드카·cron·병렬 감시자·추가 에이전트)을 막으려는 것이지, 사이클이 의존하는 확인 하나를 막으려는 게 아니었다.
  - 이 확인은 **같은 오퍼레이터가 자기 루프를 재개하는 것**이라 새 에이전트 표면이 아니다. 스폰 승인 게이트의 승인 요건이 적용되지 않는다(§0.1 carve-out).
  - **장전 먼저.** 선반이 빈 채로 걸면 매 회차 무의미한 회색만 본다.
  - 멈출 때 스케줄을 걸었거나 선반 소진을 보고했거나 둘 중 하나가 아니면, 사이클이 미완이다.

- **사이클로 돈다 (2026-07-29)**: `장전 → 버튼 확인 → 프리플라이트 → Generate 1회 → 카드 확인 → 다음 장전` 을 더 못 돌 때까지 반복한다.
  - **장전이 먼저, 버튼 확인이 나중이다.** 빈 컴포저는 큐 상태와 무관하게 항상 회색이라, 장전 전에 버튼을 보면 "생성 불가"로 오판한다.
  - **카드 확인은 턴의 끝이 아니라 다음 장전의 신호**다. 슬롯이 남아 있으면 즉시 다음 패키지로 간다.
  - **회색만이 대기 사유**다. 장전된 상태에서 회색이면 그때가 진짜 큐 만석이고, 스케줄러를 거는 시점도 그때다. 선반이 비어 있는 채로 스케줄러를 걸지 않는다.
  - 종료는 셋 중 하나이고 어느 것인지 명시한다: **큐 만석(장전 완료·대기)** / **선반 소진** / **남은 항목 전부 블로킹(각 사유)**.

- **사용자가 명시한 것이 기본값을 이긴다 (2026-07-29)**: 모드·레퍼런스 개수·길이·비율·오디오·프로바이더 모두. 기본값은 사용자가 *말하지 않은* 것에만 적용된다.
  - `멀티레퍼런스`는 **모드**(Runway의 Keyframe 반대편 탭)이고 이 파이프라인의 기본 모드다. 사용자가 "멀티레퍼런스로"라고 하면 그건 모드 지시이지 이미지 장수에 대한 언급이 아니다.
  - **개수는 별개다.** 요청에 따르며 보통 3~4장. *에이전트가* 임의로 개수를 고정하거나 채우지 않는다는 뜻이지, 사용자가 말한 개수를 무시하라는 뜻이 아니다.
...
- **PRE-ARM 필수 (버튼 감시의 전제조건)**: Generate 버튼은 큐 만석뿐 아니라 **컴포저가 비어 있어도 회색**이다. 빈 보드를 감시하면 슬롯이 풀려도 영원히 회색 = 무한대기.
  ...
  - **옵저버 지시문은 손으로 쓰지 않고 생성한다**: `python3 runtime/scripts/runway_ui_helper.py observer-instruction --project <p>` 출력을 그대로 예약한다. 손으로 쓴 지시문이 같은 이유로 두 번 실패했다 — 한 번은 `E24`와 확인할 한글 문장을 박아 8회 연속 같은 결함만 확인했고, 한 번은 `조건이 하나라도 없으면 BLOCKED를 알린다`라는 자체 실패 의미를 정의해 **Generate 후 8초에 정지**하고(규칙은 60초 폴링) 새로고침·피드 확인을 건너뛰고 스케줄러까지 멈췄다.
  - 반복 지시문은 매 회차 자기 텍스트를 다시 읽는다. 거기 담긴 정지 조건은 영구화되고, 에이전트는 받은 과업을 따르므로 그 문구가 스킬 규칙을 조용히 이긴다. **생성해서 쓰는 것이 둘을 동기화하는 유일한 방법이다.**
  - 지시문이 표현하지 못하는 제약이 필요하면 그건 장전된 패키지나 프로젝트 상태에 넣는다. 반복 텍스트에 넣지 않는다.
