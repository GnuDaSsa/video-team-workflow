...
이 파일이 영상팀의 **유일한 에이전트 규칙 원본**이다. 다른 문서(스킬, 위키, `/Users/gnudas/Documents/Codex/video-team-runtime/runtime/references/*)`는 보조 자료거나 기계용 자산이며, 이 파일과 충돌하면 이 파일이 이긴다. 규칙 반영 여부 점검은 이 파일 하나만 보면 된다. 새 규칙은 반드시 여기에 먼저 쓰고, 다른 곳엔 포인터만 남긴다.
