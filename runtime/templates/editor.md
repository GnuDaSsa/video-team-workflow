...
- Use only clips in `media/07_videos_approved_영상승인/`. In verified `visual_only_no_audio` mode, do not add a music/VO track or create a placeholder audio asset; preserve only appropriate diegetic sound embedded in approved clips and never claim music synchronization. Otherwise use locked audio in `media/02_audio_음악/`.
