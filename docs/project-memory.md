...
- Default recurring-character identity asset is `CHAR_<ID>_TRIPTYCH_R<n>`: left headless front body (or recorded full-head fallback after one failed targeted edit), middle back body with head, right large 3/4 portrait as primary face authority. Bind actual roles explicitly in the model-facing prompt, use deterministic crops for fragile shots, keep state changes as separate assets, and require three varied reference-bound checks before dependent production plus real clip identity QC. A 10-case study is optional, not the default gate.
